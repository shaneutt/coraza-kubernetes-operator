...
func TestRuleSetReconciler_ReconcileConfigMaps(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			assert.False(t, result.Requeue)
			entry, ok := ruleSetCache.Get(tt.instanceName)
...
func TestRuleSetReconciler_MissingConfigMap(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, result.RequeueAfter > 0, "Should requeue when ConfigMap is not found")
	_, ok := ruleSetCache.Get(testInstance)
	assert.False(t, ok)
}
...
func TestRuleSetReconciler_ValidationRejection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ruleSet.Spec.Instance = testInstance
...
func TestRuleSetReconciler_UpdateCache(t *testing.T) {
	...
	entry1, _ := ruleSetCache.Get("update-test")
	uuid1 := entry1.UUID
	...
	entry2, _ := ruleSetCache.Get("update-test")
